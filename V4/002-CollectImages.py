...
def get_image(code):
        ...
        # return np.random.randint(255, size=(384,512))
        ...
        try:
            ...
            print('\nFailed to download image, waiting 10 seconds.')
            time.sleep(10)
            r = requests.get(url)
...
def make_dataset(dataset_name, save_dir, codes_fname, start, count):
        datafile = h5py.File(os.path.join(save_dir, dataset_name + ".hdf5"), "w")
        codenames = np.genfromtxt(os.path.join(Dir,codes_fname), dtype='str')
        print('\nDownloading files and writing them to HDF.')
        imsshape = (count, 384, 512)
        image_set = datafile.create_dataset("images", imsshape, chunks=True, compression='gzip', compression_opts=2)
        amazoncodes = datafile.create_dataset("amazoncodes", (count,), dtype=h5py.string_dtype(encoding='ascii'), compression='gzip', compression_opts=2)
        # amazoncodes = datafile.create_dataset("amazoncodes", dtype=h5py.string_dtype(encoding='ascii'), data=[s.encode('ascii') for s in codes], compression='gzip', compression_opts=2)
        for i in range(count):
            ims = get_image(codenames[i])
            image_set[i,...] = np.array(ims, dtype='f8')
            amazoncodes[i] = [codenames[i].encode('ascii'),]
            datafile.flush()

        # # ims, codes = get_img_array(codes_fname, start, step_size)
        # # Save the data in 1000 image chunks so we don't chew all our RAM up.
        # for i in range(int(len(ims)/100)):
        #     off = i*100
        #     print(f'Saving {off} to {off+100-1}.')
        #     image_set[off:off+100,...] = np.array(ims[off:off+100], dtype='f8')
        # if len(ims)%100:
        #     off = int(len(ims)/100)*100
        #     print(f'Saving {off} to {len(ims)-1}.')
        #     image_set[off:,...] = np.array(ims[off:], dtype='f8')
        # Save the data in 1000 code chunks so we don't chew all our RAM up.
        # for i in range(int(len(codes)/1000)):
        #     off = i*1000
        #     print(f'Saving codes {off} to {off+1000-1}.')
        #     amazoncodes[off:off+1000] = [s.encode('ascii') for s in codes[off:off+1000]]
        # if len(codes)%1000:
        #     off = int(len(codes)/1000)*1000
        #     print(f'Saving codes {off} to {len(codes)-1}.')
        #     amazoncodes[off:] = [s.encode('ascii') for s in codes[off:]]
        datafile.close()
...
if __name__ == '__main__':
    ...
    make_dataset(args.hdffile, Dir, args.txtfile, args.start_number, args.count)
