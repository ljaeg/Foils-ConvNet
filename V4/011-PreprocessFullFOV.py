...
# @njit()
def ArrayFromShuffledIndexes(Source, Dest, Shape, Indices):
    for j, i in enumerate(Indices):
        if j % 100:
            print(f'Progress {j/float(len(Indices))*100:0.2f}%', end='\r', flush=True)
        Dest[j,...] = Source[i,...]
    print('Progress 100.00%')
    return Dest

def SplitTVT(SplitRatios, Data, OutputData, TVTNames):
    ...
    print(f"Writing {TVTNames[0]}")
    Train  =  ArrayFromShuffledIndexes(Data,  OutputData[TVTNames[0]], (len(IndTrain),  Data.shape[1],  Data.shape[2]),  IndTrain)
    print(f"Writing {TVTNames[1]}")
    Val    =  ArrayFromShuffledIndexes(Data,  OutputData[TVTNames[1]], (len(IndVal),    Data.shape[1],  Data.shape[2]),  IndVal)
    print(f"Writing {TVTNames[2]}")
    Test   =  ArrayFromShuffledIndexes(Data,  OutputData[TVTNames[2]], (len(IndTest),   Data.shape[1],  Data.shape[2]),  IndTest)
...
def CombineHDFsIntoTVT(SplitRatios, InputYesFileName, InputNoFileName, OutputFileName):
    ...
    # Calculate how many images are in each split.
    SplitSizes = np.array(SplitRatios)*InputData['images'].shape[0]
    # Split it and dump it.
    print("Splitting Yes set into Train/Val/Test.")
    OutputData.create_dataset("TrainYes", shape=(SplitSizes[0], InputData['images'].shape[1], InputData['images'].shape[2]), chunks=(1,FOVSize[0],FOVSize[1]), compression='gzip', compression_opts=2)
    OutputData.create_dataset("ValYes",   shape=(SplitSizes[1], InputData['images'].shape[1], InputData['images'].shape[2]), chunks=(1,FOVSize[0],FOVSize[1]), compression='gzip', compression_opts=2)
    OutputData.create_dataset("TestYes",  shape=(SplitSizes[2], InputData['images'].shape[1], InputData['images'].shape[2]), chunks=(1,FOVSize[0],FOVSize[1]), compression='gzip', compression_opts=2)
    TrainYes, ValYes, TestYes = SplitTVT(SplitRatios, InputData['images'], OutputData, TVTNames=['TrainYes', 'ValYes', 'TestYes'])
    ...
    SplitSizes = np.array(SplitRatios)*InputData['images'].shape[0]
    # Split it and dump it.
    OutputData.create_dataset("TrainNo", shape=(SplitSizes[0], InputData['images'].shape[1], InputData['images'].shape[2]), chunks=(1,FOVSize[0],FOVSize[1]), dtype='f8', compression='gzip', compression_opts=2)
    OutputData.create_dataset("ValNo",   shape=(SplitSizes[1], InputData['images'].shape[1], InputData['images'].shape[2]), chunks=(1,FOVSize[0],FOVSize[1]), compression='gzip', compression_opts=2)
    OutputData.create_dataset("TestNo",  shape=(SplitSizes[2], InputData['images'].shape[1], InputData['images'].shape[2]), chunks=(1,FOVSize[0],FOVSize[1]), compression='gzip', compression_opts=2)
    TrainNo, ValNo, TestNo = SplitTVT(SplitRatios, InputData['images'], OutputData, TVTNames=['TrainNo', 'ValNo', 'TestNo'])
...
if __name__ == '__main__':
    ...
    OutputDir = os.path.join(Dir, '10000_384x512')
    ...
    InputYesFileName = os.path.join(Dir, 'amazon10k_FullFOV_yes.hdf5')
    InputNoFileName = os.path.join(Dir, 'amazon10k_FullFOV_no.hdf5')
