...
def GetGlobbedFiles(InputFileName):
    ### SCAN THE RAW DATA
...
def blanks_do(FOVSize, FullFOV, NumFOVs, DataDir, InputYesFileName, InputNoFileName, OutputFileName, daskclient):
    ...
    GlobbedFilesYes = GetGlobbedFiles(InputYesFileName)
    GlobbedFilesNo = GetGlobbedFiles(InputYesFileName)
    ...
    compression_opts = 2
    TrainYes = DataFile.create_dataset('TrainYes', (NumTrain, FOVSize[0], FOVSize[1]), dtype='f8', chunks=True, compression='gzip', compression_opts=compression_opts)
    TrainNo  = DataFile.create_dataset('TrainNo',  (NumTrain, FOVSize[0], FOVSize[1]), dtype='f8', chunks=True, compression='gzip', compression_opts=compression_opts)

    TestYes  = DataFile.create_dataset('TestYes', (NumTest, FOVSize[0], FOVSize[1]), dtype='f8', chunks=True, compression='gzip', compression_opts=compression_opts)
    TestNo   = DataFile.create_dataset('TestNo',  (NumTest, FOVSize[0], FOVSize[1]), dtype='f8', chunks=True, compression='gzip', compression_opts=compression_opts)

    ValYes   = DataFile.create_dataset('ValYes', (NumVal, FOVSize[0], FOVSize[1]), dtype='f8', chunks=True, compression='gzip', compression_opts=compression_opts)
    ValNo    = DataFile.create_dataset('ValNo',  (NumVal, FOVSize[0], FOVSize[1]), dtype='f8', chunks=True, compression='gzip', compression_opts=compression_opts)
